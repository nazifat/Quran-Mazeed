  ...
  return (
    <div className='p-10'>
      ...
      <h1 className="text-xl font-bold text-center mb-6">
        <span className='font-hafs block my-2'>{suraDataArabic?.name}</span>
        Surah {suraDataArabic?.englishName}
      </h1>
      <p className='text-center my-2'>Page No. {page}</p>

      <div className='grid grid-cols-1 text-right w-full md:w-3/4 mx-auto'>
        {cleanedAyahs.map(ayah => (
          <div key={ayah.numberInSurah}>
            <div className='flex justify-between items-center'>
              {suraAudio.length > 0 && (
                <button
                  ...
                  className="bg-[#15B3B6] text-white px-2 py-1 rounded"
                >
                  {playingAyahNumber === ayah.numberInSurah ? <FaPause /> : <FaPlay />}
                </button>
              )}
              <p className="text-lg">
                <span className="font-hafs text-2xl">{ayah.text}</span>
                <span className="mx-2 px-3 py-1 bg-[#AEE6F5] rounded-full">
                  {ayah.numberInSurah}
                </span>
              </p>
            </div>
            ...
            {/* English */}
            {ayahsEnglish.filter(e => e.numberInSurah === ayah.numberInSurah).map(e => (
              <p key={e.numberInSurah} className="text-left">{e.text}</p>
            ))}

            {/* Bangla */}
            {ayahsBangla.filter(b => b.numberInSurah === ayah.numberInSurah).map(b => (
              <p key={b.numberInSurah} className="text-left">{b.text}</p>
            ))}

            <hr className="border-t border-gray-200 mt-3 py-2" />
          </div>
        ))}
      </div>
