                    ...
                    ayahs?.map((ayah) => {
                        ...
                        return (
                            <div key={ayah.number} className=''>
                               ...
                               {/* Surah heading and Basmala — stays block, separate */}
...
{/* Inline ayahs container */}
<div
  dir="rtl"
  lang="ar"
  className="text-lg text-gray-800 dark:text-gray-100 leading-relaxed mb-4 md:py-5 py-0"
>
  {ayahs.map((ayah, idx) => (
    <span
      key={ayah.number}
      className="inline font-taha text-2xl leading-[2]"
      style={{ whiteSpace: 'normal' }}
    >
      {ayah.text}
      <span
        className="mx-1 px-2 py-0.5 bg-[#AEE6F5] text-[#4F888B] rounded-full text-xs font-bold border border-[#4F888B] shadow-sm font-[Scheherazade] align-middle"
        style={{ display: 'inline-block', verticalAlign: 'middle' }}
      >
        {ayah.numberInSurah}
      </span>{' '}
      {/* Add a small space after each ayah for readability */}
    </span>
  ))}
</div>
